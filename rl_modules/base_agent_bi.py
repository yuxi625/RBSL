...
from mpi_utils.mpi_utils import sync_networks, sync_grads, discounted_return, cost_return
...
class BaseAgent:
    def __init__(self, args, env, env_params):
        ...
        self.safety_her_module = her_sampler(self.args.safety_replay_strategy, self.args.safety_relabel_percent, compute_reward)
        # create the replay buffer
        self.buffer = replay_buffer(self.env_params, self.args.buffer_size, self.her_module.sample_her_transitions)
        self.safety_buffer = replay_buffer(self.env_params, self.args.safety_buffer_size, self.safety_her_module.sample_her_transitions)
    ...
    def _update_safety_network(self):
        pass
    ...
    def sample_batch_safety(self, future_p=None):
        sample_batch = self.safety_buffer.sample(self.args.batch_size, future_p=future_p)
        transitions = sample_batch['transitions']
        # pre-process the observation and goal
        # o, o_next, g, = transitions['obs'], transitions['obs_next'], transitions['g']
        o, o_next, g, = transitions['obs'], transitions['obs_next'], transitions['g']
    ...
    def _eval_agent(self, make_gif=False, epoch=0):
        total_obs, total_g, total_ag, total_rewards, total_cost, total_success_rate = [], [], [], [], [], []
        for i in range(self.args.n_test_rollouts):
            per_obs, per_g, per_ag, per_rewards, per_cost, per_success_rate = [], [], [], [], [], []
            ...
            for _ in range(self.env_params['max_timesteps']):
                ...
                observation_new, reward, cost, _, info = self.env.step(actions)
        ...
        costs = cost_return(total_cost)

        local_costs = np.mean(costs)
        global_costs_return = MPI.COMM_WORLD.allreduce(local_costs, op=MPI.SUM)
        ...
        results = {'Test/final_distance': global_distances, 
                   ...
                   'Test/costs_return': global_costs_return,
    ...
    def learn(self, evaluate_agent=True):
        ...
        if self.args.expert_percent == 0.:
            ...
            self.safety_buffer.load_filter(os.path.join(load_path_random, f'{buffer_name}.pkl'))
        elif self.args.random_percent == 0.:
            self.buffer.load(os.path.join(load_path_expert, f'{buffer_name}.pkl'))
            if self.args.env_id == "FetchReachObstacle-v0":
                self.safety_buffer.load(os.path.join(load_path_expert, f'{buffer_name}.pkl'))
            else:
                self.safety_buffer.load_filter(os.path.join(load_path_expert, f'{buffer_name}.pkl'))
        else:
            ...
            self.args)
            if self.args.env_id == "FetchReachObstacle-v0":
                self.safety_buffer.load_mixture(os.path.join(load_path_expert, f'{buffer_name}.pkl'), os.path.join(load_path_random, f'{buffer_name}.pkl'), self.args.expert_percent, self.args.random_percent,
            self.args)
            else:
                self.safety_buffer.load_filter(os.path.join(load_path_expert, f'{buffer_name}.pkl'), self.args.expert_percent)
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if evaluate_agent:
                ...
                if MPI.COMM_WORLD.Get_rank() == 0:
                    ...
                    wandb.log(results)
                    success_rates.append(results['Test/success_rate'])
                    cost_returns.append(results['Test/costs_return'])
                    print('[{}] epoch is: {}, eval success rate is: {:.3f}, final_distance is: {:.3f}, discounted_return is: {:.3f}, costs_return is: {:3f}'.format(datetime.now(), epoch, results['Test/success_rate'], results['Test/final_distance'], results['Test/discounted_return'], results['Test/costs_return']))
            ...
            for _ in tqdm(range(self.args.n_cycles)):
                # train policy
                for _ in range(self.args.n_safety_batches):
                    results3 = self._update_safety_network(mutip, future_p=future_p)
                    wandb.log(results3)
                for _ in range(self.args.n_batches):
                    results2 = self._update_network(future_p=future_p)
                    wandb.log(results2)
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            best_suc = max(success_rates)
            # index = success_rates.index(best_suc)
            max_indices = [index for index, item in enumerate(success_rates) if item == best_suc]
            cost_new = [cost_returns[i] for i in max_indices]
            best_cost = min(cost_new)

            re = [best_suc, best_cost]
